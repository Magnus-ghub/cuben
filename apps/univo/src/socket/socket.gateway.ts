...
import { OnGatewayInit, SubscribeMessage, WebSocketServer, WebSocketGateway } from '@nestjs/websockets';
...
export class SocketGateway implements OnGatewayInit {
  ...
  try {
    ...
    let { token } = parseUrl.query;

    if (!token) return null;

    // URL kodlangan bo'lsa, uni asl holiga qaytaramiz
    const decodedToken = decodeURIComponent(token as string);
    
    this.logger.debug(`Decoded Token check: ${decodedToken.substring(0, 10)}...`);
    
    return await this.authService.verifyToken(decodedToken);
  } catch (err: any) {
