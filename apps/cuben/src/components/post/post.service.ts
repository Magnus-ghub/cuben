...
import { lookupAuthMemberFollowed, shapeIntoMongoObjectId } from '../../libs/config';
...
export class PostService {
            ...
            { $sort: sort },
            {
                $facet: {
                    list: [
                        ...
                        // 3. POST EGASINI FOLLOW QILGANMI (Post ob'ektida)
                        lookupAuthMemberFollowed({
                            followerId: memberId,
                            followingId: '$memberId',
                        }),
                        
                        // 4. meLiked ni qo'shish
                        {
                            $addFields: {
                                meLiked: {
                        ...
                        // 5. MEMBER DATA LOOKUP (PIPELINE BILAN!) 🔥
                        {
                            $lookup: {
                                from: 'members',
                                let: { postMemberId: '$memberId' },
                                pipeline: [
                                    {
                                        $match: {
                                            $expr: { $eq: ['$_id', '$$postMemberId'] }
                                        }
                                    },
                                    // Member uchun meLiked
                                    {
                                        $lookup: {
                                            from: 'likes',
                                            let: { 
                                    ...
                                    // Member uchun meFollowed
                                    {
                                        $lookup: {
                                            from: 'follows',
                                            let: {
                                                localFollowerId: memberId,
                                                localFollowingId: '$_id',
                                            },
                                            pipeline: [
                                                {
                                                    $match: {
	...
	public shapeMatchQuery(match: T, input: PostsInquiry): void {
		...
		if (text) {
			match.$or = [{ postTitle: { $regex: text, $options: 'i' } }, { postContent: { $regex: text, $options: 'i' } }];
		}
	}
